defmodule Day13 do
  ...
  def part2(file) do
    ...
    h =
      blocks |> Enum.map(fn block -> block |> h_symmetry(&find_near_symmetry/1) end) |> Enum.sum()

    v =
      blocks |> Enum.map(fn block -> block |> v_symmetry(&find_near_symmetry/1) end) |> Enum.sum()
  ...
  defp do_find_symmetry([next | rest], [last | other_previous] = previous) when next == last do
    if confirm_symmetry(rest, other_previous),
      do: length(previous),
      else: do_find_symmetry(rest, [next | previous])
  end

  defp do_find_symmetry([next | rest], previous), do: do_find_symmetry(rest, [next | previous])

  defp confirm_symmetry(remaining, previous),
    do: Enum.zip(remaining, previous) |> Enum.all?(fn {a, b} -> a == b end)
  ...
  defp do_find_near_symmetry([next | rest] = list, previous) do
    if check_near_symmetry(list, previous),
      do: length(previous),
      else: do_find_near_symmetry(rest, [next | previous])
  end

  defp check_near_symmetry(list, previous),
    do:
      Enum.zip(list, previous)
      |> Enum.map(fn {x, y} -> Levenshtein.distance(x, y) end)
      |> Enum.sum() == 1
end
