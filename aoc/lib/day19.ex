defmodule Resources do
  ...
  def spend(resources, cost)
      when resources.ore >= cost.ore and
             resources.clay >= cost.clay and
             resources.obsidian >= cost.obsidian and
...
defmodule Robots do
  ...
  def try_build(robots, resources, cost, :geode) do
    turns =
      max(
        (((cost.ore - resources.ore) / robots.ore) |> Float.ceil() |> max(0) |> round()) + 1,
        (((cost.obsidian - resources.obsidian) / robots.obsidian)
         |> Float.ceil()
         |> max(0)
         |> round()) + 1
      )
...
defmodule RobotFactory do
  ...
  def new(description), do: description |> String.split(": ") |> parse()
  ...
  defp parse_robots([ore_robot, clay_robot, obsidian_robot, geode_robot]),
    do:
      {parse_robot(ore_robot), parse_robot(clay_robot), parse_robot(obsidian_robot),
       parse_robot(geode_robot)}

  defp parse_robot(<<"Each ore robot costs ", cost::bytes-size(1), " ore">>),
    do: %Resources{ore: String.to_integer(cost)}

  defp parse_robot(<<"Each clay robot costs ", cost::bytes-size(1), " ore">>),
    do: %Resources{ore: String.to_integer(cost)}

  defp parse_robot(<<"Each obsidian robot costs ", cost::binary>>) do
    %{"ore" => ore, "clay" => clay} =
      Regex.named_captures(
        ~r/(?<ore>\d+) ore and (?<clay>\d+) clay.*/,
        cost
      )
  ...
  defp parse_robot(<<"Each geode robot costs ", cost::binary>>) do
    %{"ore" => ore, "obsidian" => obsidian} =
      Regex.named_captures(
        ~r/(?<ore>\d+) ore and (?<obsidian>\d+) obsidian.*/,
        cost
      )
  ...
  defp max_geodes(factory, resources, robots, turns_remaining) do
    cond do
      too_many_robots?(factory, robots) ->
        0

      max_potential(resources, robots, turns_remaining) < BestResult.value() ->
        0

      true ->
        build_options(factory, resources, robots)
