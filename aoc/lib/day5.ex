defmodule Day5 do
  ...
  defp parse_initial_state(state), do:
    do_parse_initial_state(state |> String.split("\n") |> Enum.reverse())
  ...
  defp do_parse_moves([head | tail], moves), do: do_parse_moves(tail, [ head |> String.split() |> tl |> Enum.take_every(2) |> Enum.map(&String.to_integer/1) | moves ])

  defp apply_moves([state, moves]), do: do_apply_moves(state, moves)
  defp do_apply_moves(state, []), do: state
  defp do_apply_moves(state, [[count, from, to] | other_moves]), do: do_apply_moves(do_apply_move(state, from, to, count), other_moves)

  defp do_apply_move(state, _, _, 0), do: state
  defp do_apply_move(state, from, to, count), do:
    do_apply_move(%{ state | to => [ hd(state[from]) | state[to] ], from => tl(state[from])}, from, to, count - 1)

  defp apply_moves_without_reordering([state, moves]), do: do_apply_moves_without_reordering(state, moves)
  defp do_apply_moves_without_reordering(state, []), do: state
  defp do_apply_moves_without_reordering(state, [[count, from, to] | other_moves]), do:
    do_apply_moves_without_reordering(
      %{ state | to => Enum.take(state[from], count) ++ state[to], from => Enum.drop(state[from], count) },
      other_moves
    )
