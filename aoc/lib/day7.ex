...
defmodule Day7 do
  def part1(file),
    do:
      ...
      |> Enum.reduce_while(0, fn x, acc ->
        if x < 100_000, do: {:cont, x + acc}, else: {:halt, acc}
      end)
  ...
  defp sizes(acc, directory, []) do
    dir_size =
      Enum.sum(directory.files) +
        Enum.sum(Enum.map(Map.values(directory.subdirectories), fn x -> x.size end))
  ...
  defp sizes(acc, directory, [<<?$, ?\s, ?c, ?d, ?\s, dir_name::binary>> | tail]) do
    {updated_acc, subdir, remaining_commands} =
      sizes(acc, directory.subdirectories[dir_name], tail)

    updated_subdirectories = Map.put(directory.subdirectories, dir_name, subdir)

    sizes(
      updated_acc,
      %Directory{directory | subdirectories: updated_subdirectories},
      remaining_commands
    )
  end
