defmodule Day8 do
  ...
  defp scan_line(visible, max_columns, line, row_index), do:
    do_scan_line(visible, [], max_columns, -1, String.to_charlist(line) |> Enum.with_index, row_index)

  defp do_scan_line(visible, processed, max_columns, _, [], _), do: {visible, processed, max_columns}
  defp do_scan_line(visible, processed, max_columns, row_max, [{tree, index} | tail], row_index) do
    col_max = Map.get(max_columns, index, -1)
    visible = if tree > row_max or tree > col_max, do: MapSet.put(visible, {index, row_index}), else: visible
  ...
  defp scan_view_line(scores, cols, line, row_index), do:
    do_scan_view_line(scores, [], cols, [], String.to_charlist(line) |> Enum.with_index, row_index)
  ...
  defp do_scan_view_line(scores, processed, cols, row, [{tree, index} | tail], row_index) do
    ...
    scores = Map.put(scores, {row_index, index}, Map.get(scores, {row_index, index}, 1) * col_score * row_score)
    do_scan_view_line(scores, [{tree, index} | processed], Map.put(cols, index, [tree | Map.get(cols, index, [])]), [tree | row], tail, row_index)
  end
