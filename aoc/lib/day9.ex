defmodule Day9 do
  ...
  defp do_chase(results, head, tail, [{_, 0} | other_commands]),
    do: do_chase(results, head, tail, other_commands)
  ...
  defp long_chase(commands),
    do: do_long_chase(MapSet.new(), Enum.map(1..10, fn _ -> {0, 0} end), commands)

  defp do_long_chase(results, _, []), do: results

  defp do_long_chase(results, knots, [{_, 0} | other_commands]),
    do: do_long_chase(results, knots, other_commands)

  defp do_long_chase(results, [head | knots], [{direction, distance} | other_commands]) do
    head = move_head(head, direction)
    knots = Enum.scan(knots, head, &move_tail/2)

    do_long_chase(MapSet.put(results, List.last(knots)), [head | knots], [
      {direction, distance - 1} | other_commands
    ])
  end
